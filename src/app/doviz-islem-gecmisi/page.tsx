...
import type { DovizIslemi, Sube } from "@/types/api";
...
export default function DovizIslemGecmisiPage() {
  ...
  useEffect(() => {
    ...
    dovizIslemleriniGetir(subeKodu || undefined)
      .then((data) => {
        if (!iptalEdildi) setIslemler(data);
      })
      .catch((error: unknown) => {
        if (!iptalEdildi) {
