...
import SubeDetayPopup from "@/components/subeler/SubeDetayPopup";
...
export default function SubelerPage() {
  ...
  const [detaySubeKodu, setDetaySubeKodu] = useState<string | null>(null);
  ...
  return (
    <AppShell>
                ...
                {filtrelenmisSubeler.map((sube) => (
                  <tr key={sube.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <button type="button" onClick={() => setDetaySubeKodu(sube.kod)} className="rounded-md border border-blue-200 px-3 py-2 text-xs font-semibold text-[#0047b3] hover:bg-blue-50">Detay</button>
                    </td>
                  </tr>
                ))}
                {yukleniyor && (
      ...
      <SubeDetayPopup
        key={detaySubeKodu ?? "kapali"}
        isOpen={detaySubeKodu !== null}
        subeKodu={detaySubeKodu}
        onClose={() => setDetaySubeKodu(null)}
      />
    </AppShell>
  );
}
